...
def parse_args(argv):
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Init/update qifqif categories json file by importing "
    ...
    parser.add_argument(
        ...
        "--config",
        dest="config",
        help=("configuration filename to update. " "DEFAULT: ~/.qifqif.json"),
        default=os.path.join(os.path.expanduser("~"), ".qifqif.json"),
    )
    parser.add_argument(
        "csv", metavar="CSV_FILE", help=".CSV accounts file to process", default=""
    )
    parser.add_argument(
        "-d",
        "--delimiter",
    ...
    parser.add_argument(
        ...
        dest="dry_run",
        action="store_true",
        help=("dry-run mode: just print " "instead of writing file"),
    )
    parser.add_argument(
        "field",
        metavar="FIELD",
        type=int,
        help=("index of field to extract"),
        default=False,
    )
...
def main(argv=None):
    ...
    accounts = read_accounts(args["csv"], args["field"] - 1, args["delimiter"])
